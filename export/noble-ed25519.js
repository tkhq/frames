...
var nobleEd25519 = (() => {
  ...
  // only exporting what we need for getting the public key
  var input_exports = {};
  ...
  const sha512s = (
    ...m // Sync SHA512, not set by default
  ) =>
  ...
  const verifyAsync = async (s, m, p, opts = dvo) =>
    hashFinish(true, _verify(s, m, p, opts));
  const verify = (s, m, p, opts = dvo) =>
