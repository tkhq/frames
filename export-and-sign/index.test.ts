...
import { TKHQ } from "./src/turnkey-core.js";
...
beforeAll(async () => {
  Object.defineProperty(window, "__TURNKEY_SIGNER_ENVIRONMENT__", {
    value: "prod",
  });
...
describe("TKHQ", () => {
  describe("LocalStorage with expiry", () => {
    it("gets and sets items with expiry localStorage", async () => {
      ...
      const itemA = JSON.parse(window.localStorage.getItem("a")!);
  ...
  describe("Embedded key management", () => {
    ...
    it("gets and sets embedded key in localStorage", async () => {
      ...
      // Set a dummy "key"
      TKHQ.setEmbeddedKey({ alg: "bar" });
      expect(TKHQ.getEmbeddedKey()).toEqual({ alg: "bar" });
    });
  ...
  describe("Key generation and cryptography", () => {
    it("generates P256 keys", async () => {
      const key = await TKHQ.generateTargetKey();
  ...
  describe("Misc util functions", () => {
    ...
    it("contains uint8arrayFromHexString", () => {
      ...
      //
      expect(() => {
        // @ts-expect-error Error case: bad value
  ...
  describe("Settings and styles validation", () => {
    it("validates styles", async () => {
      const simpleValid1 = { padding: "10px" };
      expect(TKHQ.validateStyles(simpleValid1)).toEqual(simpleValid1);
      ...
      expect(TKHQ.validateStyles(simpleValid2)).toEqual(simpleValid2);

      const simpleValidPadding = {
        "padding  ": "10px",
        margin: "10px",
        fontSize: "16px",
      };
      expect(TKHQ.validateStyles(simpleValidPadding)).toEqual(simpleValid2);

      const simpleInvalidCase = {
        padding: "10px",
        margin: "10px",
        "font-size": "16px",
      };
...
describe("Event Handler Expiration Flow", () => {
  ...
  it("blocks signing after expiry", async () => {
    ...
    try {
      ...
      TKHQ.sendMessageUp("ERROR", String(e), requestId);
    }
  ...
  it("clears only the expired key when multiple keys exist", async () => {
    ...
    try {
      ...
      expect(String(e)).toContain("key bytes have expired");
    }
    ...
    try {
      ...
      expect(String(e)).toContain("key bytes not found");
    }
  ...
  it("clears expired key even when accessed with default address", async () => {
    ...
    try {
      ...
      expect(String(e)).toContain("key bytes not found");
      expect(String(e)).not.toContain("expired");
    }
  });
});
...
describe("EVM Signing", () => {
  ...
  async function injectEvmKey(address: string | undefined) {
    const HpkeDecryptMock = jest
      .fn()
...
describe("Embedded Key Override", () => {
  ...
  describe("SET_EMBEDDED_KEY_OVERRIDE handler", () => {
    ...
    it("uses injected key for subsequent bundle decryptions", async () => {
      ...
      expect(secondCall.receiverPrivJwk).not.toEqual({ alg: "bar" }); // not the embedded key
  ...
  describe("RESET_TO_DEFAULT_EMBEDDED_KEY handler", () => {
    it("clears the injected embedded key", async () => {
      ...
      expect(lastCall.receiverPrivJwk).toEqual({ alg: "bar" }); // back to the embedded key
    });
  });
  ...
  describe("Full Lifecycle", () => {
    it("replace key -> inject bundles -> sign -> reset -> inject uses embedded key", async () => {
      ...
      const lastCall =
        HpkeDecryptMock.mock.calls[HpkeDecryptMock.mock.calls.length - 1][0];
      expect(lastCall.receiverPrivJwk).toEqual({ alg: "bar" }); // embedded key
    });
  });
});
...
describe("Channel Establishment Guard", () => {
  ...
  beforeEach(async () => {
    window.document.body.innerHTML = MINIMAL_INIT_HTML;

    jest.spyOn(TKHQ, "sendMessageUp").mockImplementation(() => {});
    jest
      .spyOn(TKHQ, "setParentFrameMessageChannelPort")
      .mockImplementation(() => {});
    jest.spyOn(TKHQ, "initEmbeddedKey").mockResolvedValue(undefined);
    jest
      .spyOn(TKHQ, "getEmbeddedKey")
      .mockReturnValue({ kty: "EC", crv: "P-256" });
    jest
      .spyOn(TKHQ, "p256JWKPrivateToPublic")
      .mockResolvedValue(new Uint8Array(65).fill(0x04));
    jest.spyOn(TKHQ, "uint8arrayToHexString").mockReturnValue("aabbccdd");
  });
  ...
  it("processes a single TURNKEY_INIT_MESSAGE_CHANNEL and sends PUBLIC_KEY_READY", async () => {
    ...
    expect(TKHQ.initEmbeddedKey).toHaveBeenCalledTimes(1);
    expect(TKHQ.setParentFrameMessageChannelPort).toHaveBeenCalledTimes(1);
    expect(TKHQ.sendMessageUp).toHaveBeenCalledWith(
      "PUBLIC_KEY_READY",
      "aabbccdd"
    );
  });
  ...
  it("ignores a second TURNKEY_INIT_MESSAGE_CHANNEL dispatched while the first is still pending", async () => {
    ...
    expect(TKHQ.initEmbeddedKey).toHaveBeenCalledTimes(1);
    expect(TKHQ.setParentFrameMessageChannelPort).toHaveBeenCalledTimes(1);
    expect(TKHQ.setParentFrameMessageChannelPort).toHaveBeenCalledWith(port1);
    expect(TKHQ.setParentFrameMessageChannelPort).not.toHaveBeenCalledWith(
      port2
    );
