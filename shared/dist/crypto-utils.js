...
async function HpkeDecrypt({
  ...
  const kemContext = new core.DhkemP256HkdfSha256();
  ...
  var suite = new core.CipherSuite({
    kem: kemContext,
  ...
  var receiverPubBuf = await turnkeyCore.p256JWKPrivateToPublic(receiverPrivJwk);
  var aad = turnkeyCore.additionalAssociatedData(encappedKeyBuf, receiverPubBuf);
  var res;
  try {
...
async function HpkeEncrypt({ plaintextBuf, receiverPubJwk }) {
  ...
  const aad = turnkeyCore.additionalAssociatedData(encappedKeyBuf, receiverPubBuf);
  ...
  const ciphertextHex = turnkeyCore.uint8arrayToHexString(new Uint8Array(ciphertextBuf));
  const encappedKeyBufHex = turnkeyCore.uint8arrayToHexString(encappedKeyBuf);
